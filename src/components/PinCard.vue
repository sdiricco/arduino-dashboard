<template>
  <v-sheet v-for="(pin, i) in board.firmata.pins" class="pa-4 ma-4 d-flex flex-column align-center justify-center"
    rounded>
    <v-chip>{{ i }}</v-chip>
    <v-switch inset color="secondary" hide-details @click="onClickSwitch($event, i)"
      :model-value="Boolean(pin.value)"></v-switch>
  </v-sheet>
</template>
